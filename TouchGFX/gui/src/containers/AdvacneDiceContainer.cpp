#include <gui/containers/AdvacneDiceContainer.hpp>
#include "BitmapDatabase.hpp"
#include <math.h>

AdvacneDiceContainer::AdvacneDiceContainer() :
...
void AdvacneDiceContainer::initialize()
{
    AdvacneDiceContainerBase::initialize();
...
void AdvacneDiceContainer::handleTickEvent()
{
...
void AdvacneDiceContainer::left()
{
    animating = true;
    steps = 100;
    previousY = 0;
...
void AdvacneDiceContainer::right()
{
    animating = true;
    steps = 100;
    previousY = 0;
...
void AdvacneDiceContainer::down()
{
    animating = true;
    steps = 100;
    previousZ = 0;
...
void AdvacneDiceContainer::setScale(float value)
{
    for (int i = 0; i < NUMBER_OF_DICE_SIDES; i++)
    {
        diceSides[i].invalidate();
...
void AdvacneDiceContainer::reorderCube()
{
    for (int i = 0; i < NUMBER_OF_DICE_SIDES; i++)
    {
        remove(diceSides[i]);
...
void AdvacneDiceContainer::updateRightToLeft(TextureMapper& tm)
{
...
void AdvacneDiceContainer::updateLeftToRight(TextureMapper& tm)
{
...
void AdvacneDiceContainer::updateUpToDown(TextureMapper& tm)
{
...
void AdvacneDiceContainer::updateDownToUp(TextureMapper& tm)
{
    tm.updateZAngle(tm.getZAngle() + PI);
...
void AdvacneDiceContainer::resetCubeSidesAngles()
{
    for (int i = 0; i < NUMBER_OF_DICE_SIDES; i++)
    {
...
void AdvacneDiceContainer::setDiceSides(uint16_t leftFrontBitmap)
{
...
void AdvacneDiceContainer::enter3Dcube(float simpleDiceAngle)
{
    for (int i = 0; i < NUMBER_OF_DICE_SIDES; i++)
    {
        switch (i)
...
void AdvacneDiceContainer::exit3Dcube()
{
    steps = 100;
    previousY = 0;
    startValueY = 0;
...
bool AdvacneDiceContainer::getIsAnimating()
{
    return animating;
}

uint16_t AdvacneDiceContainer::getLeftFrontBitmapId()
{
    return cubeMap[LEFT_FRONT];
}

float AdvacneDiceContainer::getFrontBitmapAngle()
{
    return diceSides[0].getZAngle();
}

float AdvacneDiceContainer::floatBackEaseOut(float t, float b, float c, float d)
{
    float s = 1.70158f;
    t = t / d - 1;
...
float AdvacneDiceContainer::floatBackEaseIn(float t, float b, float c, float d)
{
    float s = 1.70158f;
    float postFix = t /= d;
...
float AdvacneDiceContainer::floatBackEaseInOut(float t, float b, float c, float d)
{
    float s = 1.70158f;
    if ((t /= d / 2) < 1)
    {
...
float AdvacneDiceContainer::easeNone(float t, float b, float c, float d)
{
    return c * t / d + b;
}
